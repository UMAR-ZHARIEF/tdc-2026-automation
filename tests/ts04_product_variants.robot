...
...               cannot be manipulated on a live store). Products (re-pointed live after a
...               catalogue-wide products.json lead scan + live PDP verification of
...
TC-04-002 Single-Variant Product Proceeds Without Explicit Selection
    ...
    ...    re-pointed live: 'Striped top' (striped-top), a Shopify 'Default Title'
    ...    single-variant product, live-verified in the same pass to render NO variant selector and an
...
TC-04-007 Size And Colour Are Selectable Together
    ...
    ...    Specification's mode: Executable). Product re-pointed live: 'Noir jacket'
    ...    (noir-jacket), Size S/M/L x Color Blue/Red with all six combinations available,
    ...    live-verified in the same pass to render both dimensions with 2+ options. The shared
