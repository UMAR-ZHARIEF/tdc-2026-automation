...
...               TC-13-001..007. Authorities: 02 - Test Case Specification v2.1 and 01 - Test
...               Basis v1.0 (approved 2026-08-05), UC-13 (TB-CONF-001..008). 5 cases automated,
...               ALL gated; 2 documented SKIP (TC-13-005/006: 02 v2.1 §MODES Design-only — a
...
...               therefore PROVISIONAL, built only from the verified TEXT evidence in 02 v2.1
...
TC-13-001 Display Confirmation Page
    ...
    [Tags]    priority-high    type-positive    TC-13-001    TB-CONF-001    TB-CONF-002    TB-CONF-003    TB-CONF-004    TB-CONF-005    TB-CONF-006    TB-CONF-007    TB-CONF-008
...
TC-13-002 Verify The Confirmation Number Is Displayed
    ...
    [Tags]    priority-high    type-positive    TC-13-002    TB-CONF-002
    Skip If    not ${ALLOW_ORDERS}    Requires a completed order — run only in the explicitly authorized combined TS-12->TS-13 session (team payment policy, order budget).
...
TC-13-003 Verify The Purchased-Item Summary And Totals
    ...
    [Tags]    priority-high    type-positive    TC-13-003    TB-CONF-006
    Skip If    not ${ALLOW_ORDERS}    Requires a completed order — run only in the explicitly authorized combined TS-12->TS-13 session (team payment policy, order budget).
...
TC-13-007 Confirmation E-Mail Delivery
    ...
    [Tags]    priority-medium    type-positive    TC-13-007    TB-CONF-008
    Skip If    not ${ALLOW_ORDERS}    Requires a completed order — run only in the explicitly authorized combined TS-12->TS-13 session (team payment policy, order budget).
...
TC-13-004 Continue Shopping
    ...
    [Tags]    priority-medium    type-positive    TC-13-004    TB-CONF-001    TB-CONF-002    TB-CONF-003    TB-CONF-004    TB-CONF-005    TB-CONF-006    TB-CONF-007    TB-CONF-008
    Skip If    not ${ALLOW_ORDERS}    Requires a completed order — run only in the explicitly authorized combined TS-12->TS-13 session (team payment policy, order budget).
...
TC-13-005 Confirmation Page Fails (Design-Only)
    ...
    [Tags]    priority-critical    type-negative    design-only    TC-13-005    TB-CONF-001    TB-CONF-002    TB-CONF-003    TB-CONF-004    TB-CONF-005    TB-CONF-006    TB-CONF-007    TB-CONF-008
...
TC-13-006 Confirmation Completeness — Missing Receipt Information (Design-Only)
    ...
    [Tags]    priority-critical    type-negative    design-only    TC-13-006    TB-CONF-002    TB-CONF-003    TB-CONF-004    TB-CONF-005    TB-CONF-006
