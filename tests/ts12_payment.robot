...
...               control; TC-12-005: automation-limited, settled by the Phase H
...
...               2 = declined, 3 = gateway failure (verified by executed transactions
...               live). Cards 2 and 3, and any invalid/unrecognised value, create NO order under
...
...               with an unconditional documented Skip (automation-limited, settled by a later
...               manual retest) and retains the same ALLOW_ORDERS gate directly behind it as
...
...               question was settled by ONE manual-retest order, #V860ENJ8W); team payment
...               policy applies; only run authorized sessions.
...               New page object: resources/pages/checkout_page.resource (captured live): see
...               that file's own Documentation for capture scope and the
...
...               ⚠️ SUPERSEDED in a later revision: the state-chained design described below is no
...
...               Shopify checkout session open for retry (verified live) so no
...
...               Traffic (rewritten for the per-case Test Setup design): each taken
...
...               /cart/clear rule above belongs to the superseded chained design; a later
...               campaign ran the per-case shape cleanly.) TC-12-004 and TC-12-005 carry
...
TC-12-002 Payment Declined
    ...
    ...    use a different
    ...    payment method." No order is created; user remains on checkout. Verified live.
    ...
    ...    filled). Card fields are observed to CLEAR after a failed attempt (verified live);
    ...    logged as evidence here, and acted on by TC-12-006/003/005, which each
...
TC-12-006 Payment Gateway Timeout
    ...
    ...    Verified live. SAFETY: card "3" is store-published as gateway-failure-only: it
    ...    creates NO order under any circumstance and is default-executable. Card fields are
    ...    re-entered here because TC-12-002's failed attempt cleared them (verified live).
...
TC-12-003 Invalid Card Information
    ...
    ...    attempt cleared them (verified live). Chained from TC-12-006 (deliberate: same
...
TC-12-001 Successful Payment
    ...
    ...    order confirmation page displays with a confirmation number. Verified live
...
TC-12-005 Retry Payment After Failure
    ...
    ...    them) with card 1.
    ...    2. Pay. Expected: retry succeeds. Verified live. SAFETY INVARIANT: the published
    ...
    ...    all card fields since they clear after the failure (verified live). See this
    ...    file's Documentation for the full order-budget note (this was the second of up to two
    ...    real orders when both gated cases executed). RESOLUTION: settled by the
